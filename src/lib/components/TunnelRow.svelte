<script lang="ts">
  ...
  const dotText  = $derived(statusText[tunnel.status] ?? 'text-zinc-400');
  const isToken  = $derived(store.selectedProject?.auth_mode === 'token');
</script>

<!-- ── Card layout — small screens ─────────────────────────────────────────── -->
<div class="md:hidden border-b border-base-300/60 last:border-0 px-4 py-3 space-y-2
    ...
    <div class="flex items-center gap-2 min-w-0">
      ...
      {#if tunnel.target_type}
        <span class="badge badge-sm flex-shrink-0
                     {tunnel.target_type === 'local' ? 'badge-info' : 'badge-warning'} opacity-70">
          {tunnel.target_type}
        </span>
      {/if}
    </div>
    <span class="text-[10px] flex-shrink-0 {dotText}">{tunnel.status}</span>
  </div>

  <!-- Row 2: hostname + actions -->
  <div class="flex items-center gap-1.5 min-w-0">
    <span class="text-xs font-mono text-base-content/50 truncate flex-1">
      {tunnel.hostname ?? '—'}
    </span>
    {#if tunnel.hostname}
      <button onclick={copyHostname} title={copied ? 'Copied!' : 'Copy hostname'} class="flex-shrink-0">
        {#if copied}
          <Check class="w-3.5 h-3.5 text-emerald-400" strokeWidth={2.5} />
        {:else}
          <Copy class="w-3.5 h-3.5 text-base-content/40" strokeWidth={2} />
        {/if}
      </button>
      <button onclick={() => api.openUrl('https://' + tunnel.hostname)}
              title="Open in browser" class="flex-shrink-0">
        <ExternalLink class="w-3.5 h-3.5 text-base-content/40" strokeWidth={2} />
      </button>
    {/if}
  </div>

  <!-- Row 3: service + namespace -->
  {#if editing}
    <div class="flex items-center gap-1">
      <input type="text" bind:value={editService}
        class="input input-xs flex-1 bg-base-100 border-base-300 font-mono text-[10px]
               focus:outline-none focus:ring-1 focus:ring-primary/30" />
      <button class="btn btn-xs btn-primary px-2" onclick={saveEdit}>
        <Check class="w-3 h-3" strokeWidth={2.5} />
      </button>
      <button class="btn btn-ghost btn-xs px-1.5" onclick={() => (editing = false)}>
        <X class="w-3 h-3" strokeWidth={2} />
      </button>
    </div>
  {:else}
    <div class="flex items-center gap-2 text-[10px] font-mono text-base-content/35">
      <span class="truncate flex-1">{tunnel.service ?? '—'}</span>
      {#if tunnel.namespace}
        <span class="flex-shrink-0 text-base-content/25">· {tunnel.namespace}</span>
      {/if}
    </div>
  {/if}
  ...
  {#if confirming}
    <div class="flex items-center gap-1.5">
      ...
      <button class="btn btn-error btn-xs gap-1" onclick={teardown} disabled={store.busy}>
        {#if store.busy}
      ...
      <button class="btn btn-ghost btn-xs" onclick={() => (confirming = false)} disabled={store.busy}>
        <X class="w-3 h-3" strokeWidth={2} />
      </button>
    </div>
  {:else}
    <div class="flex items-center gap-1 justify-end">
      {#if isToken}
        <button class="btn btn-ghost btn-xs text-base-content/40 hover:text-primary hover:bg-primary/10"
                onclick={() => (editing = true)} disabled={store.busy} title="Edit service URL">
          <Pencil class="w-3 h-3" strokeWidth={2} />
        </button>
      {/if}
      <button class="btn btn-ghost btn-xs text-base-content/40 hover:text-red-400 hover:bg-red-400/10 gap-1"
              onclick={() => (confirming = true)} disabled={store.busy}>
...
<!-- ── Table row — md+ screens ──────────────────────────────────────────────── -->
<div class="hidden md:flex items-center gap-3 px-4 py-2.5 border-b border-base-300/60 last:border-0
  ...
  <div class="flex items-center gap-1 flex-1 min-w-0">
    ...
    {#if tunnel.hostname}
      <button class="opacity-0 group-hover:opacity-60 hover:!opacity-100 transition-opacity flex-shrink-0"
              onclick={copyHostname} title={copied ? 'Copied!' : 'Copy hostname'}>
        {#if copied}
      ...
      <button class="opacity-0 group-hover:opacity-60 hover:!opacity-100 transition-opacity flex-shrink-0"
              onclick={() => api.openUrl('https://' + tunnel.hostname)} title="Open in browser">
  ...
  <!-- Status label -->
  <span class="text-[10px] w-14 text-right {dotText}">{tunnel.status}</span>
    ...
    {#if confirming}
      <input type="text" bind:value={pendingNs}
        class="input input-xs w-20 bg-base-100 border-base-300 font-mono text-[10px]
               focus:outline-none focus:ring-1 focus:ring-red-500/30"
        placeholder="namespace" title="K8s namespace to delete" />
      <button class="btn btn-error btn-xs px-2" onclick={teardown}
              disabled={store.busy} title="Confirm teardown">
        {#if store.busy}
          <span class="loading loading-spinner loading-xs"></span>
        {:else}
          <Check class="w-3 h-3" strokeWidth={2.5} />
      ...
      <button class="btn btn-ghost btn-xs px-1.5 text-base-content/40"
              onclick={() => (confirming = false)} disabled={store.busy}>
        <X class="w-3 h-3" strokeWidth={2} />
      </button>
    {:else}
      {#if isToken}
        <button class="btn btn-ghost btn-xs opacity-0 group-hover:opacity-100 transition-opacity
                ...
                onclick={() => (editing = true)} disabled={store.busy}
